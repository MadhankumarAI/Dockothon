...
const CreateEntry = () => {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState(null);
  const [showDropdown, setShowDropdown] = useState(false);
  const [topViewVideo, setTopViewVideo] = useState(null);
  const [bottomViewVideo, setBottomViewVideo] = useState(null);
  const [videoPreviews, setVideoPreviews] = useState({ top: null, bottom: null });
  const [uploadProgress, setUploadProgress] = useState({ top: 0, bottom: 0 });
  const [formData, setFormData] = useState({
    amount_voided: '',
    diameter_of_commode: '',
    notes: '',
  ...
  useEffect(() => {
    ...
    // Clean up video preview URLs on unmount
    return () => {
      if (videoPreviews.top) URL.revokeObjectURL(videoPreviews.top);
      if (videoPreviews.bottom) URL.revokeObjectURL(videoPreviews.bottom);
    };
  }, []);
  ...
  const handleVideoChange = (e, viewType) => {
    ...
    if (file) {
      ...
      const allowedTypes = [
        ...
        'video/mpg', 'video/mpeg', 'video/quicktime'
      ];
      
      const fileExtension = file.name.split('.').pop().toLowerCase();
      const allowedExtensions = ['mp4', 'mov', 'avi', 'mkv', 'webm', 'flv', 'wmv', 'm4v', 'mpg', 'mpeg'];
      
      if (!allowedTypes.includes(file.type) && !allowedExtensions.includes(fileExtension)) {
        setError('Invalid file type. Please upload a video file (MP4, MOV, AVI, MKV, etc.)');
        return;
      }
      ...
      if (viewType === 'top') {
        setTopViewVideo(file);
        setVideoPreviews(prev => ({ ...prev, top: previewUrl }));
        setUploadProgress(prev => ({ ...prev, top: 0 }));
      } else {
        setBottomViewVideo(file);
        setVideoPreviews(prev => ({ ...prev, bottom: previewUrl }));
        setUploadProgress(prev => ({ ...prev, bottom: 0 }));
      }
  ...
  const clearVideo = (viewType) => {
    if (viewType === 'top') {
      if (videoPreviews.top) URL.revokeObjectURL(videoPreviews.top);
      setTopViewVideo(null);
      setVideoPreviews(prev => ({ ...prev, top: null }));
      setUploadProgress(prev => ({ ...prev, top: 0 }));
      if (topVideoRef.current) topVideoRef.current.value = '';
    } else {
      if (videoPreviews.bottom) URL.revokeObjectURL(videoPreviews.bottom);
      setBottomViewVideo(null);
      setVideoPreviews(prev => ({ ...prev, bottom: null }));
      setUploadProgress(prev => ({ ...prev, bottom: 0 }));
      if (bottomVideoRef.current) bottomVideoRef.current.value = '';
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!topViewVideo && !bottomViewVideo && !formData.amount_voided && !formData.diameter_of_commode && !formData.notes) {
      setError('Please provide at least one piece of information (video, measurement, or notes)');
      return;
    }
    ...
    try {
      ...
      formDataToSend.append('doctor_id', selectedDoctor.id);
      
      if (topViewVideo) {
        formDataToSend.append('top_view_video', topViewVideo);
      }
      if (bottomViewVideo) {
        formDataToSend.append('bottom_view_video', bottomViewVideo);
      }
      
      if (formData.amount_voided) {
        formDataToSend.append('amount_voided', parseFloat(formData.amount_voided));
      }
      if (formData.diameter_of_commode) {
        formDataToSend.append('diameter_of_commode', parseFloat(formData.diameter_of_commode));
      }
      if (formData.notes) {
        formDataToSend.append('notes', formData.notes);
      }

      // Use the entryAPI with FormData
      const response = await entryAPI.create(formDataToSend);
      ...
      setError(err.response?.data?.detail || err.response?.data?.message || 'Failed to create entry');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-900">Video Uploads (Optional)</h3>
                <p className="text-sm text-gray-500 mb-4">Upload videos for analysis. At least one video or measurement is required.</p>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          ref={topVideoRef}
                          accept="video/*"
                          onChange={(e) => handleVideoChange(e, 'top')}
                          className="hidden"
                          id="top-video-input"
                        />
                        <label
                          htmlFor="top-video-input"
                          ...
                          <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4">
                            {videoPreviews.top ? (
                              <>
                                ...
                                <p className="mb-2 text-sm text-emerald-600 font-medium">
                                  Top view video selected
                                </p>
                                <p className="text-xs text-emerald-500">Click to change video</p>
                              </>
                            ) : (
                              <>
                                ...
                                <p className="text-xs text-gray-500">MP4, MOV, AVI, MKV, etc.</p>
                    ...
                    {videoPreviews.top && (
                      <div className="relative">
                        ...
                        {uploadProgress.top > 0 && uploadProgress.top < 100 && (
                          <div className="mt-2">
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-emerald-500 transition-all duration-300"
                                style={{ width: `${uploadProgress.top}%` }}
                              ></div>
                            </div>
                            <p className="text-xs text-gray-600 text-center mt-1">
                              Uploading: {uploadProgress.top}%
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          ref={bottomVideoRef}
                          accept="video/*"
                          onChange={(e) => handleVideoChange(e, 'bottom')}
                          className="hidden"
                          id="bottom-video-input"
                        />
                        <label
                          htmlFor="bottom-video-input"
                          className={`flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer transition-colors duration-300 ${
                            videoPreviews.bottom 
                              ? 'border-emerald-300 bg-emerald-50' 
                              : 'border-gray-300 bg-gray-50 hover:bg-gray-100'
                          }`}
                        >
                    ...
                    {videoPreviews.bottom && (
                      <div className="relative">
                        ...
                        <div className="relative rounded-lg overflow-hidden border border-gray-200">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-3">
                            <p className="text-white text-sm font-medium">
                              {bottomViewVideo?.name}
                            </p>
                          </div>
                        </div>
                        {uploadProgress.bottom > 0 && uploadProgress.bottom < 100 && (
                          <div className="mt-2">
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-emerald-500 transition-all duration-300"
                                style={{ width: `${uploadProgress.bottom}%` }}
                              ></div>
                            </div>
                            <p className="text-xs text-gray-600 text-center mt-1">
                              Uploading: {uploadProgress.bottom}%
                            </p>
                          </div>
                        )}
                      </div>
              ...
              {/* Measurements */}
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-900">Measurements (Optional)</h3>
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Amount Voided (ml)
              ...
              <div className="bg-gradient-to-r from-[rgb(193,218,216)]/10 to-emerald-50 rounded-xl p-4 border border-[rgb(193,218,216)]/30">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Videos are securely uploaded to Cloudinary with end-to-end encryption.
                      All information is HIPAA compliant and only your selected doctor can access this data.
                    </p>
                  </div>
                </div>
              </div>
